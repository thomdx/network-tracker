...
db.query(`SELECT COALESCE(o.traffic_source, c.traffic_source, 'Unknown') as name, COUNT(o.id) as sales, COALESCE(SUM(o.payout),0) as revenue FROM orders o LEFT JOIN clicks c ON o.click_id=c.click_id WHERE 1=1 ${df} ${uf} GROUP BY COALESCE(o.traffic_source, c.traffic_source), o.traffic_source, c.traffic_source ORDER BY revenue DESC`, params)
    ]);
